...
An application program interface (API) was developed to streamline the processing of Eddy Covariance (EC) data in Eddy Pro&reg;.  The API consists of two modules: the Pre-processing module allows users to filter raw EC data using quality controls, generate descriptive statistics for the raw data, and check for changes in metadata (e.g., instrument separation) and group the EC data into periods common configurations.  The Main module then takes the results of the pre-processing routine and uses them to run EddyPro in parallel batches, to minimize computation times.  Preliminary testing indicates the EddyPro API can speed up processing by 67% compared using the EddyPro GUI.  Furthermore, the API allows for automation of EC data processing while maintaining the ability to use the EddyPro GUI to define and inspect processing configurations.
